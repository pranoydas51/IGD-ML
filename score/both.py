...
def get_inception_and_fid_score(images, labels, fid_cache, num_images=None,
    ...
    if FLAGS.prd: #and len(fid_acts)==50000:
        ...
        if FLAGS.data_type == "cifar100" or FLAGS.data_type == "cifar100lt":
           feats = np.load('/depot/gupta869/data/pranoy/CBDM-pytorch/stats/cifar10_feats.npy')
        elif FLAGS.data_type == "cifar10" or FLAGS.data_type == "cifar10lt":
           feats = np.load('/depot/gupta869/data/pranoy/CBDM-pytorch/stats/cifar100_feats.npy')
        ...
        prd_data = compute_prd_from_embedding(eval_data=fid_acts,ref_data=feats,num_clusters=num_clusters,num_angles=1001,num_runs=10,enforce_balance=False)
    ...
    if FLAGS.improved_prd and len(fid_acts)==50000:
        ...
        if isinstance(fid_acts, torch.Tensor):
            fid_acts = fid_acts.numpy()
        ipr = IPR(32, k=5, num_samples=500, model='InceptionV3')
