...
FLAGS = flags.FLAGS
flags.DEFINE_float('beta', 1.0, help='weight for mutual regularization in IGD')
...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def compute_per_class_loss(self, x_0, y, augm=None):
        """
        Compute per-class loss for IGD using CBDM's calculation method, scaled by 1/n_y, with regularizer averaged over all other classes.
        """
        assert len(torch.unique(y)) == 1, "Batch must have single class"
        y_val = y[0].item()
        ...
        h = self.model(x_t, t, y=y_input, augm=augm)
        loss_ddpm = F.mse_loss(h, noise, reduction='none').mean()

        loss_reg = 0.0
        if FLAGS.igd and y_input is not None:
            # Batch all other y'
            other_y = [k for k in range(self.num_class) if k != y_val]
            num_other = len(other_y)
            if num_other > 0:
                b = x_0.shape[0]
                x_t_repeat = x_t.repeat(num_other, 1, 1, 1)
                t_repeat = t.repeat(num_other)
                y_prime = torch.tensor(other_y, device=y.device).unsqueeze(1).repeat(1, b).view(-1)
                augm_repeat = augm.repeat(num_other, 1) if augm is not None else None
                h_prime_all = self.model(x_t_repeat, t_repeat, y=y_prime, augm=augm_repeat)
                weight = (t.float()[:, None, None, None] / self.T) * FLAGS.beta  # [b, 1, 1, 1]
                weight_repeat = weight.repeat(num_other, 1, 1, 1)
                h_repeat = h.repeat(num_other, 1, 1, 1)
                mse_reg = F.mse_loss(h_repeat, h_prime_all.detach(), reduction='none')
                loss_reg = (weight_repeat * mse_reg).mean() / num_other  # average over y'

        # Scale by n_y
        n_y = self.dataset.cls_num_list[y_val] if hasattr(self.dataset, 'cls_num_list') else 1
        loss_y = (loss_ddpm + loss_reg) / n_y
